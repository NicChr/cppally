...
r_vec<r_sexp> resize_all(r_vec<r_sexp> x, r_size_t n){
    ...
    for (r_size_t i = 0; i < list_length; ++i){
        r_visit(x.view(i), [&]<RVector T>(T vec) {
...
To disambiguate the actual type we can use `r_visit()` via a C++ lambda.

**Example:** using `r_visit()` to resize every vector to length n in-place

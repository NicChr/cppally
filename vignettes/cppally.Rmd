...
```{r, include = FALSE}
required <- c("bench", "brio", "callr", "cli", "decor", "desc", "glue", "vctrs")
...
lgl_true_code <- 'r_true'
lgl_false_code <- 'r_false'
lgl_na_code <- 'r_na'

lgl_true_res <- cpp_eval(lgl_true_code)
lgl_false_res <- cpp_eval(lgl_false_code)
lgl_na_res <- cpp_eval(lgl_na_code)

as_cpp_chunk(paste(lgl_true_code, lgl_false_code, lgl_na_code, sep = "\n"))
```
...
r_vec<r_lgl> lgl_ops(){
  ...
  '
cpp_source(code = lgl_code)
as_cpp_chunk(lgl_code)
```
...
'
source_and_display(bad_print_code)
```
...
'
source_and_display(good_print_code)
```
...
'
source_and_display(also_good_print_code)
```
...
'
source_and_display(new_vec_code)
```
...
```{r, echo=FALSE, results = 'asis'}
eval_and_display('make_vec<r_dbl>(1, 1.5, 2, na<r_dbl>())')
```
...
```{r, echo=FALSE, results = 'asis'}
eval_and_display('make_vec<r_sexp>(1, 2, 3)')
```
...
'
source_and_display(all_vec_code)
```
...
Exporting variadic templates is also not supported. The best alternative is 
...
'
source_and_display(double_to_int_code)
```
...
'
source_and_display(to_int_code)
```
...
'
source_and_display(coercions_code)
```
...
```{r, echo=FALSE, results = 'asis'}
eval_and_display('r_str("hello").c_str()')
```
...
```{r, echo=FALSE, results = 'asis'}
eval_and_display('r_sym("new_symbol")')
```

Or from a cppally string

```{r, echo=FALSE, results = 'asis'}
eval_and_display('r_sym(r_str("symbol_from_string"))')
```
...
```{r, echo=FALSE, results = 'asis'}
eval_and_display('cached_str<"cached_string">()')
```
...
```{r, echo=FALSE, results = 'asis'}
eval_and_display('cached_sym<"cached_symbol">()')
```
...
'
source_and_display(new_list_code)
```
...
source_and_display(resize_code)
```
...
source_and_display(resize_code2)
```
...
'
source_and_display(factor_code)
```
...
'
source_and_display(fct_codes_code)
```
...
source_and_display(list_as_df_code)
```
...
'
source_and_display(n_unique_code, debug = FALSE)
```
...
source_and_display(primitive_sum_code, debug = FALSE)
```

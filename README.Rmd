...
int64_t cppally_as_int64_t(){
  ...
  vec_null = '
[[cppally::register]]
r_vector<r_int> vec_null(){
  return r_vector<r_int>(r_null);
}',
  vec_is_null = '
[[cppally::register]]
bool vec_is_null(){
  return r_vector<r_int>(r_null).is_null();
}',
  count_val = '
[[cppally::register]]
...
source_code_collapsed <- paste(examples[-1], collapse = "\n")
...
as_cpp_chunk(examples[["basic_sum"]])
```
...
NA values are handled like in R. In this case NA is returned if 
the vector contains one or more NA values. 
...
cppally makes heavy use of templates for powerful generic
...
There are two main limitations to be aware of. The first is that templates must
be written in header files if they are to be used across multiple
compilation units. The other big limitation is that template specialisations 
cannot be called from R, so when calling C++ template functions from R, 
we always rely on automatic deduction from the function inputs.
There is a workaround discussed in the main vignette [Getting started with
cppally](https://nicchr.github.io/cppally/articles/cppally.html)
...
Using the R C API alongside cppally is strongly discouraged for the following
reasons.
...
done in-place via `r_vector::set()`. It is up to the user to ensure that a
fresh vector is created before further manipulation or that it's safe to
modify the existing vector.
...
The double `1.2345` was implicitly converted to `1`, 
an example of **partial lossy coercion**. cppally allows this.

What cppally doesn't allow is **total lossy coercion**, which can result in 
ambiguity. Take the following example of counting the occurrence of a 
value in a vector. 

```{r, echo=FALSE, results = 'asis'}
as_cpp_chunk(examples[["count_val"]])
```
...
`as.double("1. 0")` was coerced to `NA`, `count_val()` then counted the number of
`NA` values and returned 40, even though we didn't ask for a count of `NA`, so the
result should have been 0.

This is the main issue with allowing total lossy coercion to `NA` - `count_val()` 
can't distinguish between a true `NA` and an `NA` that has been produced from a 
lossy coercion.
...
`r_vector<T>` objects can be `r_null`. To detect this, use the `is_null()`
member function.
...
Once you have both paths, set compilerPath and the R include path in c_cpp_properties.json accordingly.

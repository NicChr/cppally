...
#define OMP_SIMD_REDUCTION3(OP1, OP2, OP3) OMP_PRAGMA(omp simd reduction(OP1) reduction(OP2) reduction(OP3))
...
#define OMP_PARALLEL_FOR_SIMD_REDUCTION3(n_threads, OP1, OP2, OP3) OMP_PRAGMA(omp parallel for simd if(parallel: (n_threads) > 1) num_threads((n_threads)) reduction(OP1) reduction(OP2) reduction(OP3))

...
pak::pak("NicChr/cppally")
```

## Basic usage: Sum using a C++ template
...
NA values are handled like in R. In this case NA is returned if the
vector contains one or more NA values.
...
cppally makes heavy use of templates for powerful generic programming.
...
Using the R C API alongside cppally is strongly discouraged for the
following reasons.
...
Copy-on-modify can be enabled via
[`cppally::use_copy_on_modify()`](https://nicchr.github.io/cppally/reference/use_copy_on_modify.md)
or by setting the CPPALLY_COPY_ON_MODIFY Makevars flag directly. When
this is enabled, all in-place modifications check that the object being
modified isn’t referenced or owned by another object. If it is
referenced, a copy is taken first before modifying, otherwise it
directly modifies.
...
The benefit of cppally’s approach is that when registering C++ functions
to R, inputs can be supplied flexibly without unexpected behaviour.
...
The double `1.2345` was implicitly converted to `1`, an example of
**partial lossy coercion**. cppally allows this.

What cppally doesn’t allow is **total lossy coercion**, which can result
in ambiguity. Take the following example of counting the occurrence of a
value in a vector.
...
So far so good. But what happens if we implicitly coerced to NA and then
counted occurrences?
...
`as.double("1. 0")` was coerced to `NA`, `count_val()` then counted the
...
count_val(x, "1. 0")
#> Error:
#> ! Implicit NA coercion detected from r_str to r_dbl, please ensure data can be coerced without complete loss of information
```

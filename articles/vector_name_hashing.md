...
To speed up named-indexing, cppally implements hashing of vector names
via a lazy cache-on-lookup approach. To better understand how this
works, we will show what name-indexing in R looks like, how to improve
it with hashing, as well as the benefits and limitations of cppally’s
inherent lazy-hashing method.
...
#> 1 large[[1]]   92.9ns    103ns  9001136.        0B     900.
mark(large[[length(large)]])
#> # A tibble: 1 × 6
#>   expression                  min   median `itr/sec` mem_alloc `gc/sec`
#>   <bch:expr>             <bch:tm> <bch:tm>     <dbl> <bch:byt>    <dbl>
#> 1 large[[length(large)]]    152ns    159ns  6178555.        0B        0
```
...
#>   expression      min   median `itr/sec` mem_alloc `gc/sec`
#>   <bch:expr> <bch:tm> <bch:tm>     <dbl> <bch:byt>    <dbl>
#> 1 by_name       101ns    109ns  8476014.        0B     848.
#> 2 by_index       92ns    101ns  9684871.        0B       0
...
#>   expression      min   median `itr/sec` mem_alloc `gc/sec`
#>   <bch:expr> <bch:tm> <bch:tm>     <dbl> <bch:byt>    <dbl>
#> 1 by_name     478.5µs  501.4µs     1965.        0B        0
#> 2 by_index     92.1ns   99.1ns  9602058.        0B        0
```
...
#>   expression          min   median `itr/sec` mem_alloc `gc/sec`
#>   <bch:expr>     <bch:tm> <bch:tm>     <dbl> <bch:byt>    <dbl>
#> 1 by_name         478.5µs    501µs     1976.        0B        0
#> 2 by_index         92.9ns    101ns  8367114.        0B        0
#> 3 by_hashed_name    599ns    622ns  1083662.        0B        0
```
...
cppally registers C++ functions to R. R can only handle `SEXP` objects,
a generic type that represents any R object.
...
and the following R and C++ code is automatically generated.
...
[`.Call()`](https://rdrr.io/r/base/CallExternal.html) is called and is
passed the input `x`.
[`.Call()`](https://rdrr.io/r/base/CallExternal.html) is the R interface
that allows us to pass R objects to C++.
...
with that vector, which is then passed to `cpp_to_r()` - a function that
converts C/C++ objects back to `SEXP`.
...
[`.Call()`](https://rdrr.io/r/base/CallExternal.html)). Because
`r_vector` is the class that builds and stores the cache, that cache is
lost when converting to `SEXP` and therefore, the hashing approach
becomes difficult.
...
Here we are simply comparing the cost of doing a 1st-lookup, in both R
and C++
...
#>   expression              min   median `itr/sec` mem_alloc `gc/sec`
#>   <bch:expr>         <bch:tm> <bch:tm>     <dbl> <bch:byt>    <dbl>
#> 1 cppally_one_lookup     29µs   30.1µs    32505.        0B        0
#> 2 base_one_lookup       484µs  503.9µs     1955.    21.6KB        0
```
...
We will now plot the per-call cost of doing many repeated lookups to see
how many lookups we need to do before we start seeing the benefits of
hashing

![](vector_name_hashing_files/figure-html/unnamed-chunk-12-1.png)
...
Let’s see how this compares to a pseudo hash-on-1st lookup method. To
achieve this we will set the first lookup to be a fast linear scan of
...
![](vector_name_hashing_files/figure-html/unnamed-chunk-14-1.png)

The two lines are overlapping after a few lookups, signifying that there
is little penalty to waiting until the 2nd lookup to build the hash map.

Let’s also plot some comparisons comparing repeated hash lookups against
repeated linear scan lookups. At the same time we will also use a larger
list.
...
![](vector_name_hashing_files/figure-html/unnamed-chunk-16-1.png)

The best and worst case linear scans are presented for illustrative
purposes even though they are not realistic with real data. The most
informative comparison is between the trend of the hash lookup cost
versus the linear scan (mixed case). We can see that for the first few
lookups the linear scan dominates (as expected), but as the number of
lookups increases, the hash approach begins to outperform the linear
scan (mixed case) and stabilises well below it.

In summary, cppally’s lazy hashing trades a one-time hash build cost for
fast repeated lookups. The approach works best in C++, where the cache
persists across calls. From R, each call starts fresh, so the benefit is
limited to functions that perform multiple lookups within a single call.

## Future development

While not explored in this vignette, it is possible to preserve the
cache across the R/C++ boundary. By attaching an `externalptr` to the
`names_map` as an attribute of a custom R vector class, the cache
lifetime is tied to the lifetime of the R object (provided that
`externalptr` is kept as an attribute). This may require cppally support
for injecting a pre-built cache into a fresh `r_vector` wrapper, but is
otherwise a tractable extension.

...
vector list chunks, adding nodes is cheap (via `SET_VECTOR_ELT()`). When
an `r_sexp` is copied, we increment the reference count associated with
that node and only release the node once the reference count is 0, which
means all `r_sexp` objects pointing to that `SEXP` have been destroyed.
...
All functions have been compiled with C++20, GCC 14.2.0 with -O2
optimisations.
...
On my machine, cpp11 performs an insert & release every ~36 nanoseconds.
cppally performs significantly better, with ~16 nanoseconds per insert &
release.
...
at ~34 ns/copy and cppally at ~0.6 ns/copy.
...
#> 1 C_na_count(x)   34.4µs   63.4µs    15485.        0B        0
```
...
#>   expression             min   median `itr/sec` mem_alloc `gc/sec`
#>   <bch:expr>        <bch:tm> <bch:tm>     <dbl> <bch:byt>    <dbl>
#> 1 cpp11_na_count(x)   6.35ms   6.49ms      152.        0B     45.2
```
...
#>   expression               min   median `itr/sec` mem_alloc `gc/sec`
#>   <bch:expr>          <bch:tm> <bch:tm>     <dbl> <bch:byt>    <dbl>
#> 1 cppally_na_count(x)   1.45ms   1.46ms      683.        0B        0
```
...
#>   expression                    min   median `itr/sec` mem_alloc `gc/sec`
#>   <bch:expr>               <bch:tm> <bch:tm>     <dbl> <bch:byt>    <dbl>
#> 1 cppally_fast_na_count(x)   63.5µs   63.7µs    15443.        0B        0
```
...
#>   expression                       min   median `itr/sec` mem_alloc `gc/sec`
#>   <bch:expr>                  <bch:tm> <bch:tm>     <dbl> <bch:byt>    <dbl>
#> 1 cppally_fast_na_count_v2(x)   79.6µs   79.9µs    12260.        0B        0
```
...
returned at the end of `bad()` at which point `new_str` goes out of
